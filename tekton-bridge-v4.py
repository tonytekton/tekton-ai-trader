...
@app.route("/data/system-settings", methods=["GET"])
@require_auth
...
@app.route("/data/system-settings", methods=["POST"])
@require_auth
def update_system_settings():
    try:
        ...
        if "autoTrade" in data:
            ...
            print(f"⚙️ System Setting Changed: Auto Trade = {state['auto_trade_enabled']}")
        if "fridayFlush" in data:
            state["friday_flush_enabled"] = bool(data["fridayFlush"])
            print(f"⚙️ System Setting Changed: Friday Flush = {state['friday_flush_enabled']}")
        return jsonify({
            "success": True,
            "autoTrade": state["auto_trade_enabled"],
            "fridayFlush": state["friday_flush_enabled"]
        })
    except Exception as e:
...
def get_signals():
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT signal_uuid, symbol, signal_type, timeframe, confidence_score, created_at
            FROM signals
