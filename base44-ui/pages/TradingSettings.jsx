...
export default function TradingSettings() {
  ...
  const toggleFields = [
    ...
    { key: 'news_filter_enabled', label: 'News Filter',         hint: 'Block new trades during HIGH-impact news windows', activeColor: 'cyan' },
  ];
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between gap-3 mb-6">
        ...
        <button onClick={handleSave} disabled={saving} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all
      ...
      <div className="card-dark p-6 mb-4">
        <p className="text-xs font-semibold tracking-widest uppercase text-slate-500 mb-5">Execution Controls</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {toggleFields.map(({ key, label, hint, activeColor }) => {
            ...
            const colors = {
              ...
              cyan:    { track: active ? 'bg-cyan-500'   : 'bg-slate-700', badge: 'text-cyan-400   bg-cyan-500/10   border-cyan-500/20'   },
            };
            const c = colors[activeColor];
            return (
              <div key={key} className="flex items-center justify-between gap-4 bg-slate-800/40 rounded-lg p-4">
                <div>
      ...
      {/* Numeric fields */}
      <div className="card-dark p-6 mb-4">
        <p className="text-xs font-semibold tracking-widest uppercase text-slate-500 mb-5">Risk Configuration</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {numericFields.map(({ key, label, hint, step, suffix }) => (
            <div key={key} className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-slate-300">{label}</label>
              <div className="relative">
                <input type="number" step={step} value={form[key]} onChange={e => handleChange(key, e.target.value)} className={`w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-100 text-sm focus:outline-none focus:border-blue-500 transition-colors ${suffix ? 'pr-10' : ''}`} />
