...
export default function ManualSignal() {
  ...
  const handleFile = (f) => {
    ...
    setFile(f); setPreview(URL.createObjectURL(f)); setStatus(null);
  };
    ...
    <div className="min-h-screen p-4 md:p-8 max-w-2xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Manual Signal Drop-Zone</h1>
        </div>
      ...
      <div
        ...
        className={`card-dark rounded-xl p-8 flex flex-col items-center justify-center gap-4 transition-all duration-300 min-h-[260px] ${!preview ? 'cursor-pointer' : ''} ${dragging ? 'border-blue-500/60 bg-blue-500/5 glow-blue' : ''} ${preview ? '' : 'hover:border-slate-600 hover:bg-slate-800/30'}`}
      >
        <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={e => handleFile(e.target.files[0])} />
        {preview ? (
          <div className="relative w-full">
            <img src={preview} alt="Preview" className="w-full rounded-lg object-contain max-h-64" />
            <button onClick={e => { e.stopPropagation(); setFile(null); setPreview(null); setStatus(null); }} className="absolute top-2 right-2 p-1.5 rounded-full bg-slate-900/80 border border-slate-700 text-slate-400 hover:text-slate-200 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
          <>
            <div className="p-4 rounded-full bg-slate-800 border border-slate-700"><ImageIcon className="w-8 h-8 text-slate-500" /></div>
            <div className="text-center">
              <p className="text-sm font-semibold text-slate-300">Drop TradingView screenshot here</p>
              <p className="text-xs text-slate-600 mt-1">or click to browse — PNG, JPG, WEBP</p>
            </div>
          </>
        )}
      </div>
      ...
      {status === 'success' && (<div className="mt-4 flex items-center gap-2 text-emerald-400 text-sm"><CheckCircle className="w-4 h-4" /><span>Signal sent to ai_reasoning table. External pipeline will process it shortly.</span></div>)}
