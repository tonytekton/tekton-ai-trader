...
Deno.serve(async (req) => {
  try {
    ...
    const res = await fetch(`${bridgeUrl}/proxy/executions`, {
    ...
    // data.executions = array of open + closed trades from bridge
    // Each open trade is enriched with sl_pips, tp_pips, strategy from SQL (by bridge)
